...
df = read.csv("/home/benlitterer/Personal/Professional/gitHubThings/globalTerrorismExplorationCORRECT/clean_gtd_dataset.csv")
table(df$iday)
...
ggplot(just_chosen_regions, aes(date, group=region_txt, color=which_cluster)) + geom_freqpoly(bins=40) +labs(title="Time-Clusters Visualized", group="label", color="which cluster?")
```
...
ggplot(top_countries, aes(x=reorder(country_txt,-Freq), fill=first_cluster)) + geom_bar()+theme(axis.text.x=element_text(angle=90)) + theme(axis.text.x=element_blank(),axis.ticks.x=element_blank()) + labs(title="Countries Within Interest Regions", x="countries (names removed for readability)",fill="first cluster?")
```
